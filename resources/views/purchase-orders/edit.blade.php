<x-app-layout>
    ...
    <div class="flex items-center gap-3 mb-6 animate-in" style="animation-delay: 0.05s;">
        ...
        <div>
            <h1 class="text-xl font-bold" style="color: var(--text-primary);">Edit Purchase Order ({{ $po->po_number }})</h1>
            <p class="text-sm mt-0.5" style="color: var(--text-muted);">Ubah detail PO dari klien beserta itemnya</p>
        </div>
    </div>

    <form method="POST" action="{{ route('po.update', $po) }}" id="poForm" enctype="multipart/form-data">
        @csrf
        @method('PUT')
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                <div class="card p-6 animate-in" style="animation-delay: 0.1s;">
                    <h2 class="text-sm font-semibold mb-4 pb-3" style="color: var(--text-secondary); border-bottom: 1px solid var(--border-color);">
                        Upload / Ganti PO (PDF)
                        @if($po->file_path)
                        <span class="text-xs font-normal ml-2" style="color: var(--accent-green);">
                            (Sudah ada: {{ $po->po_number }}.pdf)
                        </span>
                        @endif
                    </h2>
                    <div id="po-dropzone"
                ...
                <div class="card p-6 relative z-30">
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <x-custom-select name="customer_id" :value="old('customer_id', $po->customer_id)" placeholder="Pilih Customer" min-width="100%" :options="collect(['' => 'Pilih Customer'])->union($customers->mapWithKeys(fn($c) => [$c->id => $c->company_name . ' (' . $c->company_code . ')']))" />
                        ...
                        <div>
                            ...
                            <input type="text" class="datepicker form-input" name="po_date" value="{{ old('po_date', $po->po_date ? $po->po_date->format('Y-m-d') : date('Y-m-d')) }}" required
                                >
                        ...
                        <div>
                            ...
                            <input type="text" class="datepicker form-input" name="due_date" value="{{ old('due_date', $po->due_date ? $po->due_date->format('Y-m-d') : '') }}"
                                >
                        </div>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea name="notes" rows="3" placeholder="Catatan atau keterangan tambahan..."
                                ...
                                onblur="this.style.borderColor='var(--border-color)'">{{ old('notes', $po->notes) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="card p-6 animate-in" style="animation-delay: 0.15s;">
                    ...
                    <div id="items-container" class="space-y-4">
                        ...
                        <!-- Item Row -->
                        <div class="item-row grid grid-cols-1 sm:grid-cols-12 gap-3 items-end {{ $index > 0 ? 'pt-3' : '' }}" {!! $index > 0 ? 'style="border-top: 1px dashed var(--border-color);"' : '' !!}>
                            <div class="sm:col-span-5">
                                <label class="block text-xs font-semibold mb-1" style="color: var(--text-secondary);">Nama Item</label>
                                <input type="text" name="items[{{ $index }}][item_name]" value="{{ $item->item_name }}" required placeholder="Cth: Laptop Dell XPS"
                                    class="item-name form-input">
                            </div>
                            <div class="sm:col-span-2">
                                <label class="block text-xs font-semibold mb-1" style="color: var(--text-secondary);">Qty</label>
                                <input type="number" name="items[{{ $index }}][quantity]" value="{{ $item->quantity }}" required min="1" oninput="calculateTotal()"
                            ...
                            <div class="sm:col-span-4">
                                ...
                                <input type="number" name="items[{{ $index }}][unit_price]" value="{{ $item->unit_price }}" required min="0" step="1000" oninput="calculateTotal()" placeholder="0"
                                    class="item-price form-input">
                            </div>
                            <div class="sm:col-span-1 text-right">
                                <button type="button" onclick="removeItem(this)" class="text-rose-500 hover:text-rose-700 p-2" {{ count($po->items) <= 1 ? 'disabled style="opacity:0.3;cursor:not-allowed;"' : '' }}>
                ...
                <div class="card p-6 animate-in" style="animation-delay: 0.2s;">
                    ...
                    <div class="space-y-3">
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="font-medium" style="color: var(--text-primary);" id="display-subtotal">Rp {{ number_format($po->subtotal, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span style="color: var(--text-muted);">PPN (11%)</span>
                            <span class="font-medium" style="color: var(--text-primary);" id="display-tax">Rp {{ number_format($po->tax_amount, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex justify-between text-base font-bold pt-3" style="border-top: 1px solid var(--border-color);">
                            ...
                            <span style="color: var(--accent-blue);" id="display-total">Rp {{ number_format($po->grand_total, 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>
    ...
    <script>
        let itemIndex = {{ count($po->items) > 0 ? count($po->items) : 1 }};
