<x-app-layout>
    ...
    <div class="flex items-center gap-3 mb-6 animate-in" style="animation-delay: 0.05s;">
        ...
        <div>
            <h1 class="text-xl font-bold" style="color: var(--text-primary);">Buat Quotation Baru</h1>
            <p class="text-sm mt-0.5" style="color: var(--text-muted);">Buat penawaran harga untuk klien</p>
        </div>
    </div>

    <form method="POST" action="{{ route('quo.store') }}" id="quoForm" enctype="multipart/form-data">
        @csrf

        @if($rfq)
        <input type="hidden" name="rfq_id" value="{{ $rfq->id }}">
        <div class="card p-4 mb-5 animate-in" style="animation-delay: 0.08s; border-left: 3px solid var(--accent-blue);">
            <div class="flex items-center gap-2 text-sm">
                <span style="color: var(--text-muted);">Berdasarkan RFQ:</span>
                <code class="text-xs px-2 py-0.5 rounded font-semibold" style="background: rgba(37,99,235,0.08); color: var(--accent-blue);">{{ $rfq->rfq_number }}</code>
                <span style="color: var(--text-muted);">— {{ $rfq->customer_name }}</span>
            </div>
            @if($rfq->items->count() > 0)
            <p class="text-xs mt-1" style="color: var(--text-muted);">{{ $rfq->items->count() }} item dari RFQ akan diisi otomatis</p>
            @endif
        </div>
        @endif
            ...
            <div class="lg:col-span-2 space-y-5">
                <div class="card p-6 relative z-30">
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <x-custom-select name="customer_id" id="customer-select" :value="old('customer_id', $rfq?->customer_id)" placeholder="Pilih Customer" min-width="100%" :options="collect(['' => 'Pilih Customer'])->union($customers->mapWithKeys(fn($c) => [$c->id => $c->company_name . ' (' . $c->company_code . ')']))" />
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <x-custom-select name="sales_id" :value="old('sales_id', $rfq?->sales_id)" placeholder="Pilih Sales Marketing" min-width="100%" :options="collect(['' => 'Pilih Sales Marketing'])->union($salesList->mapWithKeys(fn($u) => [$u->id => $u->name]))" />
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea name="notes" rows="3" placeholder="Catatan atau keterangan tambahan..."
                                ...
                                onblur="this.style.borderColor='var(--border-color)'">{{ old('notes', $rfq?->notes) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="card p-6 animate-in" style="animation-delay: 0.12s;">
                    ...
                    <div class="border-2 border-dashed rounded-xl p-6 text-center transition-colors"
                        ...
                        <p class="text-sm font-medium" style="color: var(--text-secondary);" id="dropzone-text">Klik atau tarik file PDF ke sini</p>
                        <p class="text-xs mt-1" style="color: var(--text-muted);">Max 5 MB</p>
                    </div>
                ...
                <div class="card p-6 animate-in" style="animation-delay: 0.15s;">
                    ...
                    <div id="items-container" class="space-y-4">
                        @if($rfq && $rfq->items->count() > 0)
                            ...
                            <div class="item-row grid grid-cols-1 sm:grid-cols-12 gap-3 items-end">
                                <div class="sm:col-span-4">
                                    ...
                                    <input type="text" name="items[{{ $idx }}][product_name]" required value="{{ $rfi->product_name }}" placeholder="Cth: Jasa Konsultasi" class="item-name form-input">
                                </div>
                                <div class="sm:col-span-1">
                                    <label class="block text-xs font-semibold mb-1" style="color: var(--text-secondary);">Qty</label>
                                    <input type="number" name="items[{{ $idx }}][qty]" required min="0.01" step="0.01" value="{{ $rfi->qty }}" class="item-qty form-input" oninput="calcRow(this)">
                                </div>
                                <div class="sm:col-span-1">
                                    <label class="block text-xs font-semibold mb-1" style="color: var(--text-secondary);">Satuan</label>
                                    <input type="text" name="items[{{ $idx }}][unit]" value="{{ $rfi->unit }}" placeholder="Pcs" class="item-unit form-input">
                                </div>
                                <div class="sm:col-span-3">
                                    <label class="block text-xs font-semibold mb-1" style="color: var(--text-secondary);">Harga Satuan</label>
                                    <input type="number" name="items[{{ $idx }}][unit_price]" required min="0" step="1" value="0" class="item-price form-input" oninput="calcRow(this)">
                                </div>
                                <div class="sm:col-span-2">
                        ...
                        <div class="item-row grid grid-cols-1 sm:grid-cols-12 gap-3 items-end">
                            <div class="sm:col-span-4">
                                <label class="block text-xs font-semibold mb-1" style="color: var(--text-secondary);">Nama Produk</label>
                                <input type="text" name="items[0][product_name]" required placeholder="Cth: Jasa Konsultasi" class="item-name form-input">
                            </div>
                            <div class="sm:col-span-1">
                                <label class="block text-xs font-semibold mb-1" style="color: var(--text-secondary);">Qty</label>
                                <input type="number" name="items[0][qty]" required min="0.01" step="0.01" value="1" class="item-qty form-input" oninput="calcRow(this)">
                            </div>
                            <div class="sm:col-span-1">
                                <label class="block text-xs font-semibold mb-1" style="color: var(--text-secondary);">Satuan</label>
                                <input type="text" name="items[0][unit]" placeholder="Pcs" class="item-unit form-input">
                            </div>
                            <div class="sm:col-span-3">
                                <label class="block text-xs font-semibold mb-1" style="color: var(--text-secondary);">Harga Satuan</label>
                                <input type="number" name="items[0][unit_price]" required min="0" step="1" value="0" class="item-price form-input" oninput="calcRow(this)">
                            </div>
                            <div class="sm:col-span-2">
                                <label class="block text-xs font-semibold mb-1" style="color: var(--text-secondary);">Total</label>
                                <input type="text" readonly class="item-total form-input" value="0" style="background: transparent; border-color: transparent; font-weight: 600;">
                    ...
                    <div class="mt-4 pt-3 text-right" style="border-top: 1px solid var(--border-color);">
                        ...
                        <span class="text-lg font-bold" style="color: var(--text-primary);" id="grand-total">Rp 0</span>
                    </div>
                </div>
            </div>
                ...
                <div class="card p-6 animate-in" style="animation-delay: 0.2s;">
                    ...
                    <div class="space-y-3">
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="font-medium" style="color: var(--text-primary);" id="display-qty">{{ $rfq ? $rfq->items->count() : 1 }}</span>
                        </div>
                        <div class="pt-3" style="border-top: 1px solid var(--border-color);">
                            <p class="text-xs" style="color: var(--text-muted);">Upload PDF untuk mengisi item secara otomatis dari file.</p>
                        </div>
                    </div>
                </div>
    ...
    <script>
        let itemIndex = {{ ($rfq ? $rfq->items->count() : 1) }};
        ...
        function removeFile() {
            ...
            document.getElementById('parse-status').innerHTML = '';
            document.getElementById('dropzone-text').textContent = 'Klik atau tarik file PDF ke sini';
