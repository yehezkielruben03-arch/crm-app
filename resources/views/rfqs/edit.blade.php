<x-app-layout>
    ...
    <div class="flex items-center gap-3 mb-6 animate-in" style="animation-delay: 0.05s;">
        ...
        <div>
            <h1 class="text-xl font-bold" style="color: var(--text-primary);">Edit RFQ</h1>
            <p class="text-sm mt-0.5" style="color: var(--text-muted);">
                <code class="text-xs px-2 py-0.5 rounded" style="background: #f8fafc; color: var(--text-secondary);">{{ $rfq->rfq_number }}</code>
                Ubah detail RFQ beserta itemnya
            </p>
        </div>
    </div>

    <form method="POST" action="{{ route('rfq.update', $rfq) }}" id="rfqForm" x-data="rfqForm()">
        @csrf
        @method('PUT')
        
        {{-- Baris Atas: Info RFQ + Ringkasan --}}
        <div class="grid grid-cols-1 gap-5 lg:grid-cols-3 xl:grid-cols-4 mb-5">
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div class="sm:col-span-2 grid grid-cols-1 gap-4">
                            ...
                            <div>
                                ...
                                <select name="customer_contact_id" id="contact-select" required data-selected="{{ old('customer_contact_id', $rfq->customer_contact_id) }}" onchange="checkSelectedPic()"
                        ...
                        <div>
                            ...
                            <input type="date" name="need_date" value="{{ old('need_date', $rfq->need_date ? \Carbon\Carbon::parse($rfq->need_date)->format('Y-m-d') : '') }}" required
                                style="width: 100%; padding: 0.6rem 0.75rem; border-radius: 0.75rem; font-size: 0.875rem; outline: none; background: #f8fafc; border: 1px solid #e2e8f0; color: var(--text-primary);"
                        ...
                        <div>
                            ...
                            <x-custom-select name="priority" :value="old('priority', $rfq->priority ?? 'Normal')" placeholder="Pilih Priority" :options="['Normal' => 'Normal', 'Urgent' => 'Urgent', 'High Priority' => 'High Priority']" />
                        </div>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea name="notes" rows="3" placeholder="Catatan atau keterangan tambahan..."
                                ...
                                onfocus="this.style.borderColor='var(--accent-blue)'"
                                onblur="this.style.borderColor='#e2e8f0'">{{ old('notes', $rfq->notes) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>

            {{-- Kolom Kanan: Ringkasan (kompak, di samping Informasi RFQ) --}}
            <div class="lg:col-span-1 xl:col-span-1">
                <div class="card p-5" style="background: #ffffff; border: 1px solid #e2e8f0; box-shadow: 0 24px 40px -26px rgba(15,23,42,0.12);">
                    ...
                    <div class="flex flex-col gap-2 mt-4" style="width:100%; box-sizing:border-box;">
                        <button type="submit"
                            class="w-full inline-flex items-center justify-center gap-2 text-sm font-semibold text-white rounded-xl transition-all active:scale-95 hover:-translate-y-0.5"
                            style="padding: 0.75rem 1rem; background: linear-gradient(135deg, var(--accent-blue), #1d4ed8); box-shadow: 0 4px 14px rgba(37,99,235,0.35); white-space: nowrap;">
                        ...
                        <a href="{{ route('rfq.show', $rfq) }}"
                           class="w-full inline-flex items-center justify-center gap-2 text-sm font-medium rounded-xl transition-all active:scale-95"
                           style="padding: 0.75rem 1rem; background: #ffffff; border: 1px solid #e2e8f0; color: var(--text-secondary); white-space: nowrap;">
                            Batal
                        </a>
                    </div>
                </div>
    ...
    <script>
        ...
        const OLD_ITEMS = @json($rfq->items);
        
        function customerSelect() {
            return {
                open: false,
                search: '',
                selectedId: '{{ old("customer_id", $rfq->customer_id) }}',
                ...
                init() {
                    // Trigger initial load for existing customer
                    if (this.selectedId) {
                        setTimeout(() => {
                            document.querySelector('input[name="customer_id"]').dispatchEvent(new Event('change'));
                        }, 50);
        ...
        function rfqForm() {
            ...
            if (initialType !== 'Projek' && nonProj.length === 0) nonProj.push({ id: globId++, product_name: '', qty: 1, unit: '', description: '' });
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (customerInput) {
                customerInput.addEventListener('change', function () {
                    ...
                    const selectedContactId = contactSelect.getAttribute('data-selected');
                        ...
                        .then(data => {
                            ...
                            if(window.customerContactsData.length > 0) {
                                window.customerContactsData.forEach((contact, idx) => {
                                    ...
                                    if(contact.id == selectedContactId || (!selectedContactId && (contact.is_primary || idx === 0))) {
                ...
                // Auto-trigger load contacts on page load for existing customer
