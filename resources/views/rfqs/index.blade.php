                    ...
                    <tr>
                        ...
                        <td>
                            <div class="flex items-center gap-1">
                                ...
                                @if((auth()->user()->isAdminOrAbove() || auth()->user()->isLeader()) && $rfq->canBePriceSubmitted() && !in_array($rfq->status, [\App\Models\Rfq::STATUS_GOAL, \App\Models\Rfq::STATUS_PO_PENDING_ADMIN, \App\Models\Rfq::STATUS_PO_PENDING_LEADER]))
                                <a href="{{ route('rfq.price_form', $rfq) }}" title="Kalkulasi / Edit HPP"
                                    class="p-1.5 rounded-lg transition-colors"
                                    style="color: var(--text-muted);"
                                    onmouseenter="this.style.background='rgba(5,150,105,0.08)'; this.style.color='#059669'"
                                    onmouseleave="this.style.background=''; this.style.color='var(--text-muted)'">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
